...
// MakeNamedNode creates a named node, adds it to the graph and returns the new node.
...
// ReachableNeighbors returns a list of neighbors, and in
// the case of a directed graph, only the ones which have outgoing edges to them
func (g *Graph) ReachableNeighbors(n *Node) (result []string) {
	for i := range n.node.edges {
		result = append(result, n.node.edges[i].end.Name)
	}
	return
}
